...
The audit now checks the 10,000 mixed-receipt ledger stress proof and macro
diagnostics as distinct requirements. Macro diagnostics include verified solve
rate, panel correlation rho, candidate ablation rate, and token cost per solve.
The known incomplete item is still live/model-backed execution: the checked-in
